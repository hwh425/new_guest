...
class ModelTest(TestCase):
    ...
    def setUp(self):
        Event.objects.create(id=1, name='oneplus 3 event', limit=2000, status=True,
                             address='shenzhen', start_time='2018-08-31 02:18:22')
        Guest.objects.create(id=1, real_name='biibiu', phone=14611088888,
                             email='biibiu@mail.com', sign=False, event_id=1)

    def test_event_models(self):
        result = Event.objects.get(name="oneplus 3 event")
        self.assertEqual(result.address, "shenzhen", "address error!")
        self.assertTrue(result.status)

    def test_guest_models(self):
        result = Guest.objects.get(phone='14611088888')
        self.assertEqual(result.real_name, "biibiu")
...
class GuestManageTest(TestCase):
    ...
    def setUp(self):
        ...
        Guest.objects.create(real_name="alen", phone=18611001100,email='alen@mail.com', sign=0, event_id=1)
        login_user = {'username': 'admin', 'password': 'admin123456'}
        self.client.post('/login_action/', data=login_user)  # 预先登录

    def test_add_guest_data(self):
        ''' 测试添加嘉宾 '''
        guest = Guest.objects.get(real_name="alen")
...
class SignIndexActionTest(TestCase):
    ...
    def setUp(self):
        ...
        Guest.objects.create(real_name="alen", phone=18611001100, email='alen@mail.com', sign=0, event_id=1)
        Guest.objects.create(real_name="una", phone=18611001101, email='una@mail.com', sign=1, event_id=2)
        login_user = {'username': 'admin', 'password': 'admin123456'}
